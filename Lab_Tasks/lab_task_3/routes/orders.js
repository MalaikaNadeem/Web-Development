...
// NEW ROUTE: My Orders (Protected) - Must be placed before other parameterized routes
router.get('/my-orders', requireAuth, async (req, res) => {
  try {
    ...
    // Find orders that match the current user's email
    // (since orders store customerEmail, we'll match by email)
...
router.post('/place', async (req, res) => {
  try {
    ...
    // Enhanced order data - link to user if logged in
    const orderData = {
      customerName: fullName.trim(),
      customerPhone: phone.trim(),
      customerAddress: address.trim(),
      customerEmail: email ? email.trim() : (req.session.userEmail || undefined),
      userId: req.session.userId || undefined, // Link to user if logged in
...
// Individual order detail route (Protected if user wants to see their own orders)
...
// All orders route (Admin/General - you might want to protect this too)
